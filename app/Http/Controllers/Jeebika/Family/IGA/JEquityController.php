...
use App\Models\Jeebika\Validators\JEquityValidator;
use Illuminate\Contracts\Support\Renderable;
use Illuminate\Http\JsonResponse;
use Illuminate\Http\Request;
use Illuminate\Http\Response;

class JEquityController extends Controller
{

    public function index($fid): Renderable
    {
        $family = Family::findOrFail($fid);
        return view('dashboard.jeebika.family.IGA.equity', compact('family'));
    }
    ...
    public function getList(Request $request, $fid): JsonResponse
    {
        ...
        $data = JEquity::select(JEquity::listKey())
    ...
    {
        $attributes = (new JEquityValidator())->validate($data = new JEquity(), \request()->all());
        $family = Family::findOrFail($fid);
        ...
        $attributes['status'] = JEquityStatus::Pending;
        $data->fill($attributes)->save();
        return response()->json(['success' => 'Equity has been successfully created.'], Response::HTTP_OK);
    }
    ...
    public function update($fid, $id): JsonResponse
    {
        ...
        if (JEquityStatus::Pending !== $data->status) {
            return response()->json(["message" => "Only pending equity can be confirmed."], Response::HTTP_UNPROCESSABLE_ENTITY);
        }
        $attributes = (new JEquityValidator())->validate($data, \request()->all());
        $attributes['status'] = JEquityStatus::Confirmed;
        $data->fill($attributes)->save();
        return response()->json(['success' => 'Equity has been successfully confirmed.'], Response::HTTP_OK);
    }
}
