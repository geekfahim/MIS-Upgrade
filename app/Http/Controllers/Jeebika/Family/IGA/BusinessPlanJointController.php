...
class BusinessPlanJointController extends Controller
{
    public function index($fid): Renderable
    {
        $family = Family::findOrFail($fid);
        return view('dashboard.jeebika.family.IGA.business-plan-joint', compact('family'));
    }
    ...
    public function getList(Request $request, $fid): JsonResponse
    {
        ...
        $families = Family::select('id', 'name')->whereIn('id', Jeebika::where('j_gro_id', Jeebika::firstWhere('family_id', $fid)->j_gro_id)->pluck('family_id'))->get();
    ...
    public function getElementById($fid, $id): JsonResponse
    {
        $item = JBusinessPlan::with([
            ...
            'families' => function ($builder) {
                $builder->with('family:id,name')->select('j_business_plan_id', 'family_id', 'amount', 'remarks');
            },
    ...
    public function delete($fid, $id): JsonResponse
    {
        ...
        //Partner Delete
        JBusinessPlanFamily::where('j_business_plan_id', $item->id)->delete();
        //Field Delete
    ...
    private function _businessPlanValidation(Request $request)
    {
        $request->validate([
            ...
            [
                'sourceCapitals.required' => 'Capital Partner Families are required.',
    ...
    private function _sourceValidation(Request $request)
    {
        ...
        if (!empty($sourceCapitals) && count($sourceCapitals) > 0) {
            $request->validate([
                'sourceCapitals.*.partner_id' => ['required', Rule::exists(Family::getTableName(), 'id')],
    ...
    private function _businessPlanCreate(Request $request, Family $family)
    {
        $jeebika = Jeebika::firstWhere('family_id', $family->id);
        return JBusinessPlan::create([
    ...
    private function _sourceCreate(Request $request, JBusinessPlan $businessPlan)
    {
        ...
        if (!empty($sourceCapitals) && is_array($sourceCapitals) && count($sourceCapitals) > 0) {
            foreach ($sourceCapitals as $item) {
                $businessPlan->families()->create([
                    'j_project_id' => $businessPlan->j_project_id,
                    'j_gro_id' => $businessPlan->j_gro_id,
                    'family_id' => $item['partner_id'],
                    'amount' => $item['source_amount'],
                    'remarks' => $item['source_remarks'] ?: null,
                    'created_by' => $request->input('created_by'),
                    'created_at' => now(),
                ]);
            }
