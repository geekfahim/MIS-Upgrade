<template>
    <div class="position-relative">
        <div class="card-header">
            <i class="fa fa-list"></i>Approve Verification
            ...
            <div v-show="formView">
                <div class="row justify-content-center">
                    <div class="col-lg-5 col-md-6 col-sm-8 col-xs-12">
                        <div class="form-group row justify-content-center">
                            <h6 class="text-left col-sm-12">
                                <strong>
                                    ...
                                    <span v-else-if="'Pending'===chunk.status"
                        ...
                        <div class="form-group">
                            <label class="d-block czm-xs" for="">Is Investment as per Application<span
                        ...
                        <div class="form-group">
                            <label class="d-block czm-xs" for="">Is Verified Purchased<span class="text-danger">*</span></label>
                            <div class="form-check form-check-inline">
                            ...
                            <div class="form-check form-check-inline">
                                <input id="is_verified_purchase_false" v-model.number="formData.is_verified_purchase"
                                       class="form-check-input czm-xs" name="is_verified_purchase" type="radio"
                                       value="0"/>
                ...
                <div class="table-responsive">
                    <table class="table table-responsive-sm table-sm table-striped table-hover">
                        ...
                        <tr v-for="(item, index) in outputRes.data" :key="index">
                            <td>{{ sl + index }}</td>
                            <td>{{ item.j_business_plan.business_name }}</td>
                            <td>{{ item.verified_date | dateFormat }}</td>
                            <td>{{ item.verified_by.name }}</td>
                            <td>
                            ...
                            <td><span v-if="item.total_invested_amount" class="tk">{{
...
export default {
    ...
    methods: {
        ...
        onClickView(item) {
            ...
            _this.formData.total_invested_amount = item.total_invested_amount;
            _this.formData.location_of_business = item.location_of_business;
            _this.formData.business_start_date = item.business_start_date;
            _this.formData.verified_date = item.verified_date;
            _this.formData.is_investment_as_per_application = item.is_investment_as_per_application;
            _this.formData.is_verified_purchase = item.is_verified_purchase;
            _this.formData.verified_id = item.verified_id;
            _this.formData.remarks = item.remarks;
            _this.formData.status = item.status;
            $("#verified_date").datepicker("update", _this.formData.verified_date);
            $("#business_start_date").datepicker("update", _this.formData.business_start_date);
