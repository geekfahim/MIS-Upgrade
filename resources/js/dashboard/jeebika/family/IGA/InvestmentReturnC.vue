                        ...
                        <form class="form-horizontal" v-on:submit.prevent="onSubmit">
                            <div class="form-group row justify-content-center">
                                <h6 class="text-left col-sm-12">
                                    <strong v-if="createView">Add New Investment Return</strong>
                                    <strong v-else>Confirmation Investment Return</strong>
                                </h6>
                            </div>
                            <div class="form-group">
                                <label class="czm-xs">Business Application<span class="text-danger">*</span></label>
                                <select v-model="formData.j_business_plan_id" v-validate="'required'"
                                        :class="[(vvErrors.has('j_business_plan_id') ? 'is-invalid' : '')]"
                                        class="form-control form-control-sm" data-vv-as="business application"
                                        data-vv-name="j_business_plan_id">
                                    <option value="">Select One</option>
                                    <option v-for="(item,index) in businessInvestment" :key="index" :value="item.id">
                                        {{ item.business_name }}
                                    </option>
                                </select>
                                <div v-show="vvErrors.has('j_business_plan_id')" class="invalid-feedback">
                                    {{ vvErrors.first('j_business_plan_id') }}
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="czm-xs">Collection Date<span class="text-danger">*</span></label>
                            ...
                            <div class="form-group">
                                <label class="czm-xs">Collected By<span class="text-danger">*</span></label>
                                <select v-model="formData.collector_id" v-validate="'required'"
                                        :class="[(vvErrors.has('collector_id') ? 'is-invalid' : '')]"
                                        class="form-control form-control-sm" data-vv-as="collected by"
                                        data-vv-name="collector_id">
                                    <option value="">Select One</option>
                                    <option v-for="(item,index) in fieldOfficers" :key="index" :value="item.id">
                                        {{ item.name }}
                                    </option>
                                </select>
                                <div v-show="vvErrors.has('collector_id')" class="invalid-feedback">
                                    {{ vvErrors.first('collector_id') }}
                                </div>
                            </div>
                            <div class="form-group">
                ...
                <div class="table-responsive">
                    <table class="table table-responsive-sm table-sm table-striped table-hover">
                        ...
                        <tr>
                            ...
                            <th class="sort" @click="onClickSortItems('date')">Collection Date</th>
                            <th class="sort" @click="onClickSortItems('confirmed_amount')">Investment
                                Amount
                            </th>
                            <th class="sort" @click="onClickSortItems('j_business_plan_id')">Business Plan</th>
            ...
            <div class="modal-dialog modal modal-dialog-centered" role="document">
                <div v-if="chunk" class="modal-content rounded-0">
                    <div class="modal-header">
                        <h6 id="exampleModalCenterTitle" class="modal-title">Investment Details</h6>
                            ...
                            <div class="col-md-12">
                                <div class="table-responsive">
                                    <table class="table table-borderless table-sm table-hover">
                                        <tr v-if="chunk.reference_number">
                                            <td>Reference Number</td>
                                            <td>:</td>
                                            <td>{{ chunk.reference_number }}</td>
                                        </tr>
                                        <tr v-if="chunk.date">
...
export default {
    ...
    methods: {
        ...
        getAll() {
            ...
            _this.formData = {};
            _this.formData.j_business_plan_id = "";
            _this.formData.collector_id = "";
            this.$validator.reset();
            axios.post(_this.routes.list, _this.search).then((res) => {
                if (res.data) {
                    _this.outputRes = res.data.lists;
                    _this.businessInvestment = res.data.businessInvestment;
