...
@push('scripts')
    <script>
        window.appHelper = {
            routes: {
                one: "{{ route('jeebika.feedback_final.view',['gid' => $bp->j_gro_id,'bpid'=>$bp->id]) }}",
                list: "{{ route('jeebika.feedback_final.view__list',['gid' => $bp->j_gro_id,'bpid'=>$bp->id]) }}"
            }
        };
    </script>
@endpush
@section("content")
    <main class="main">
        ...
        <div class="container-fluid">
            <div class="card card-accent-info" id="final-feedback">
...
@push('footer')
    ...
    <script
        src="{{ asset('assets/js/dashboard/jeebika/gro/business-plan/feedbacks/final.js') }}"></script>
@endpush
