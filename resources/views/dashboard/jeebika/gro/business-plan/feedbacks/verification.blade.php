...
@section('title', 'Verification | Feedbacks')
@push('header')
...
@push('scripts')
    <script>
        window.appHelper = {
            routes: {
                one: "{{ route('jeebika.feedback_verification.view',['gid' => $bp->j_gro_id,'bpid'=>$bp->id]) }}",
                list: "{{ route('jeebika.feedback_verification.view__list',['gid' => $bp->j_gro_id,'bpid'=>$bp->id]) }}"
            }
        };
    </script>
@endpush
@section("content")
    <main class="main">
        ...
        <div class="container-fluid">
            <div class="card card-accent-info" id="verification-feedback">
...
@push('footer')
    ...
    <script
        src="{{ asset('assets/js/dashboard/jeebika/gro/business-plan/feedbacks/verification.js') }}"></script>
@endpush
