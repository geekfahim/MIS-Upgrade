...
@section('title', 'Withdrawal | ' . $family->name)
@push('header')
...
@push('scripts')
    <script>
        window.appHelper = {
            routes: {
                one: "{{ route('jeebika.withdrawal.view', ['fid' => $family->id]) }}",
                list: "{{ route('jeebika.withdrawal.view__list', ['fid' => $family->id]) }}",
            }
        };
    </script>
...
@section('content')
    <main class="main">
        ...
        <div class="container-fluid">
            ...
            <div class="card card-accent-info" id="withdrawal-app">
                <withdrawal-app :gro="{{ $family }}">
                </withdrawal-app>
            </div>
        </div>
    </main>
...
@push('footer')
    ...
    <script src="{{ asset('assets/js/dashboard/jeebika/family/IGA/withdrawal.js') }}"></script>
@endpush
