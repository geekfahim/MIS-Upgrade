...
@section('title', 'Investment | ' . $family->name)
@push('header')
...
@push('scripts')
    <script>
        window.appHelper = {
            routes: {
                one: "{{ route('jeebika.investment.view', ['fid' => $family->id]) }}",
                list: "{{ route('jeebika.investment.view__list', ['fid' => $family->id]) }}",
            }
        };
    </script>
...
@section('content')
    <main class="main">
        ...
        <div class="container-fluid">
            ...
            <div class="card card-accent-info" id="investment-app">
                <investment-app :gro="{{ $family }}"></investment-app>
            </div>
        </div>
    </main>
@endsection
...
@push('footer')
    ...
    <script src="{{ asset('assets/js/dashboard/jeebika/family/IGA/investment.js') }}"></script>
@endpush
