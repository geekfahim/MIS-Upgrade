...
@section('title', 'Miscellaneous | ' . $family->name)
@push('header')
...
@push('scripts')
    <script>
        window.appHelper = {
            routes: {
                one: "{{ route('jeebika.miscellaneous.view', ['fid' => $family->id]) }}",
                list: "{{ route('jeebika.miscellaneous.view__list', ['fid' => $family->id]) }}",
            }
        };
    </script>
...
@section('content')
    <main class="main">
        ...
        <div class="container-fluid">
            ...
            <div class="card card-accent-info" id="miscellaneous-app">
                <miscellaneous-app></miscellaneous-app>
            </div>
        </div>
    </main>
@endsection
...
@push('footer')
    ...
    <script src="{{ asset('assets/js/dashboard/jeebika/family/IGA/miscellaneous.js') }}"></script>
@endpush
