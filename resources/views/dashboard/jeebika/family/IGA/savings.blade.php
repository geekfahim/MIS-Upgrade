...
@section('title', 'Savings | ' . $family->name)
@push('header')
...
@push('scripts')
    <script>
        window.appHelper = {
            routes: {
                one: "{{ route('jeebika.savings.view', ['fid' => $family->id]) }}",
                list: "{{ route('jeebika.savings.view__list', ['fid' => $family->id]) }}",
            }
        };
    </script>
...
@section('content')
    <main class="main">
        ...
        <div class="container-fluid">
            ...
            <div class="card card-accent-info" id="savings-app">
                <savings-app :gro="{{ $family }}">
                </savings-app>
            </div>
        </div>
    </main>
...
@push('footer')
    ...
    <script src="{{ asset('assets/js/dashboard/jeebika/family/IGA/savings.js') }}"></script>
@endpush
